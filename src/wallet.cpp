                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
	...
	printf("SelectCoinsMinConf start\n");
    BOOST_FOREACH(COutput output, vCoins)
    {
		...
		// DEVCOIN
		else if (n < nTargetValue + CTransaction::nMinTxFee)
        {
	...
	// DEVCOIN
	if (nBest != nTargetValue && nTotalLower >= (nTargetValue + CTransaction::nMinTxFee))
	{
        //ApproximateBestSubset(vValue, nTotalLower, nTargetValue + CENT, vfBest, nBest, 1000);
		// DEVCOIN
		ApproximateBestSubset(vValue, nTotalLower, nTargetValue + CTransaction::nMinTxFee, vfBest, nBest, 1000);
	}
	...
	// DEVCOIN
    if (coinLowestLarger.second.first &&
        ((nBest != nTargetValue && nBest < (nTargetValue + CTransaction::nMinTxFee)) || coinLowestLarger.first <= nBest))
    {
        setCoinsRet.insert(coinLowestLarger.second);
        nValueRet += coinLowestLarger.first;
    }
